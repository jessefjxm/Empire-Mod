    ...
    public class FactionColonies : ModSettings {
        public static void updateChanges() {
            ...
            if (Find.WorldObjects.SettlementAt(factionFC.capitalLocation) == null && factionFC.SoSShipCapital == false) {
                Messages.Message(
                    "请重置您的首都位置。如果重置后继续看到这个告警，请反馈这一问题。",
                    MessageTypeDefOf.NegativeEvent);
            }

            if (factionFC.taxMap == null) {
                Messages.Message("你的收税地图尚未设置。请使用派系菜单进行设置。",
            ...
            if (factionFC.updateVersion < 0.359) {
                ...
                Find.LetterStack.ReceiveLetter("Empire 模组已更新！", str, LetterDefOf.NewQuest);
            }
        }
        ...
        [DebugAction("Empire", "增加 5天 时间", allowedGameStates = AllowedGameStates.Playing)]
        ...
        [DebugAction("Empire", "增加 1年 时间", allowedGameStates = AllowedGameStates.Playing)]
        ...
        [DebugAction("Empire", "打印种族", allowedGameStates = AllowedGameStates.Playing)]
        private static void PrintRaces() {
            getPlayerColonyFaction().def.pawnGroupMakers.ForEach(maker => {
                Log.Message("交易者：" + maker.traders.Count);
                foreach (PawnGenOption option in maker.options) {
                    Log.Message("种族：" + option.kind.race.defName + ", " + option.kind.defName + ", " +
                                option.kind.isFighter + ", " + option.kind.trader + " 于 " + maker.kindDef);
                }
            });
        }

        [DebugAction("Empire", "重置所有军事部队指派", allowedGameStates = AllowedGameStates.Playing)]
        private static void resetAllMilitarySquads() {
            Log.Message("Debug - 重置所有军事部队指派");
        ...
        [DebugAction("Empire", "创建随机事件", allowedGameStates = AllowedGameStates.Playing)]
            ...
            foreach (FCEventDef evtDef in DefDatabase<FCEventDef>.AllDefsListForReading) {
                if (evtDef.isRandomEvent)
                    list.Add(new DebugMenuOption(evtDef.label, DebugMenuOptionMode.Action, delegate {
                        ...
                        Log.Message("Debug - 创建随机事件 - " + evtDef.label);
                        ...
                        if (settlementString != "") {
                            Find.LetterStack.ReceiveLetter("随机事件",
                                evtDef.desc + "\n 该事件影响了以下定居点： " +
        ...
        [DebugAction("Empire", "修复失踪的定居点", allowedGameStates = AllowedGameStates.Playing)]
        private static void checkForMissingSettlements() {
            Log.Message("Debug - 修复失踪的定居点");
        ...
        [DebugAction("Empire", "重置派系领袖", allowedGameStates = AllowedGameStates.Playing)]
        private static void resetFactionLeadeers() {
            Log.Message("Debug - 重置派系领袖");
            SoS2HarmonyPatches.ResetFactionLeaders();
        }

        [DebugAction("Empire", "攻击玩家领地", allowedGameStates = AllowedGameStates.Playing)]
        private static void attackPlayerSettlement() {
            ...
            foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>().settlements) {
                list.Add(new DebugMenuOption(settlement.name, DebugMenuOptionMode.Action, delegate {
                    Log.Message("Debug - 攻击玩家领地 - " + settlement.name);
        ...
        [DebugAction("Empire", "改变居住地防御力量", allowedGameStates = AllowedGameStates.Playing)]
                            ...
                            foreach (SettlementFC settlement in worldcomp.settlements) {
                                if (settlement.isMilitaryValid() && settlement.name != evt.settlementFCDefending.name) {
                                    list2.Add(new DebugMenuOption(
                                        settlement.name + " - " + settlement.settlementMilitaryLevel + " - 繁忙： " +
                                        settlement.isMilitaryBusySilent(), DebugMenuOptionMode.Action, delegate {
                                            if (settlement.isMilitaryBusy() == false) {
                                                Log.Message("Debug - 改变居住地防御力量 - " +
                                                            evt.militaryForceDefending.homeSettlement.name + " 到 " +
                                                            settlement.name);
        ...
        [DebugAction("Empire", "升级玩家居住地", allowedGameStates = AllowedGameStates.Playing)]
        private static void UpgradePlayerSettlement() {
            List<DebugMenuOption> list = new List<DebugMenuOption>();
            foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>().settlements) {
                list.Add(new DebugMenuOption(settlement.name, DebugMenuOptionMode.Action, delegate {
                    Log.Message("Debug - 升级玩家居住地 - " + settlement.name);
        ...
        [DebugAction("Empire", "升级玩家居住地 x5", allowedGameStates = AllowedGameStates.Playing)]
        private static void UpgradePlayerSettlementx5() {
            List<DebugMenuOption> list = new List<DebugMenuOption>();
            foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>().settlements) {
                list.Add(new DebugMenuOption(settlement.name, DebugMenuOptionMode.Action, delegate {
                    Log.Message("Debug - 升级玩家居住地 x5- " + settlement.name);
                    for (int i = 0; i < 5; i++) {
                        settlement.upgradeSettlement();
                    }
                }
        ...
        [DebugAction("Empire", "测试功能", allowedGameStates = AllowedGameStates.Playing)]
        private static void testVariable() {
            Log.Message("Debug - 测试功能 - ");
            Find.World.GetComponent<FactionFC>().roadBuilder.FlagUpdateRoadQueues();
        }

        [DebugAction("Empire", "降低玩家居住地等级", allowedGameStates = AllowedGameStates.Playing)]
        private static void DelevelPlayerSettlement() {
            List<DebugMenuOption> list = new List<DebugMenuOption>();
            foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>().settlements) {
                list.Add(new DebugMenuOption(settlement.name, DebugMenuOptionMode.Action, delegate {
                    Log.Message("Debug - 降低玩家居住地等级 - " + settlement.name);
        ...
        [DebugAction("Empire", "重置军事部队冷却", allowedGameStates = AllowedGameStates.Playing)]
        private static void ResetMilitarySquads() {
            ...
            Log.Message("Debug - 重置军事部队冷却");
            foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>().settlements) {
                settlement.returnMilitary(false);
            }
        }

        [DebugAction("Empire", "清空所有旧账单", allowedGameStates = AllowedGameStates.Playing)]
        private static void clearOldBills() {
            Find.World.GetComponent<FactionFC>().OldBills = new List<BillFC>();
        }

        [DebugAction("Empire", "清空所有事件", allowedGameStates = AllowedGameStates.Playing)]
        private static void clearAllEvents() {
            Find.World.GetComponent<FactionFC>().events = new List<FCEvent>();
        }

        [DebugAction("Empire", "清空所有账单", allowedGameStates = AllowedGameStates.Playing)]
        private static void clearAllBills() {
            Find.World.GetComponent<FactionFC>().Bills = new List<BillFC>();
        }

        [DebugAction("Empire", "放置 500 白银", allowedGameStates = AllowedGameStates.PlayingOnMap)]
        private static void placeSilverFC() {
            DebugTool tool = null;
            IntVec3 DropPosition;
            Map map;
            tool = new DebugTool("选择放置位置", delegate {
        ...
        [DebugAction("Empire", "放置 50000 白银", allowedGameStates = AllowedGameStates.PlayingOnMap)]
        ...
        public static void CallinAlliedForces(SettlementFC settlement, bool DropPod) {
            if (Find.CurrentMap.Parent is WorldSettlementFC) {
                Messages.Message("你不能将你的军队部署到另一个定居点！", MessageTypeDefOf.RejectInput);
                return;
            }
            ...
            IntVec3 dropPosition;
            var tool = new DebugTool("选择部署位置", delegate {
                dropPosition = UI.MouseCell();
                if (DropPod) {
                ...
                Find.LetterStack.ReceiveLetter("军队已部署",
                    "领地 " + settlement.name + " 的部队已部署到 " +
        ...
        public static void CallinAlliedForces(SettlementFC settlement, bool DropPod, int cost) {
            ...
            tool = new DebugTool("Select Deployment Position", delegate {
                ...
                Find.LetterStack.ReceiveLetter("军队已部署",
                    "领地 " + squad.settlement.name + " 的部队已部署到 " +
                    Find.CurrentMap.Parent.LabelCap, LetterDefOf.NeutralEvent,
        ...
        public static void FireSupport(SettlementFC settlement, MilitaryFireSupport support) {
            DebugTool tool = null;
            IntVec3 DropPosition;
            tool = new DebugTool("选择炮击位置", delegate {
                ...
                if (PaymentUtil.getSilver() > cost) {
                    ...
                    Messages.Message(support.name + " 很快就会落到标记的位置上！",
                        MessageTypeDefOf.ThreatSmall);
                    settlement.artilleryTimer = Find.TickManager.TicksGame + 60000;
                } else {
                    Messages.Message("你没有足够的白银来支付攻击！",
        ...
        private static void ArtilleryStrike() {
            ...
            tool = new DebugTool("选择炮击位置", delegate {
                ...
                Messages.Message("炮击很快就会落到标记的位置上！",
                    MessageTypeDefOf.ThreatSmall);
            ...
            foreach (SettlementFC settlement in Find.World.GetComponent<FactionFC>().settlements) {
                if (settlement.militarySquad != null) {
                    list.Add(new FloatMenuOption(settlement.name, delegate {
                        ...
                        DebugTool tool = null;
                        IntVec3 DropPosition;
                        tool = new DebugTool("选择降落地点", delegate {
                            DropPosition = UI.MouseCell();
                            parms.spawnCenter = DropPosition;
        ...
        [DebugAction("Empire", "呼叫援军", allowedGameStates = AllowedGameStates.PlayingOnMap)]
        private static void CallInAlliedForcesDebug() {
            CallInAlliedForcesSelect();
        }


        [DebugAction("Empire", "升级派系", allowedGameStates = AllowedGameStates.PlayingOnMap)]
